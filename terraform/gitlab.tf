resource "proxmox_virtual_environment_vm" "fanat-gitlab-core" {
  count = 1
  
  name      = "fanat-gitlab-core"
  node_name = "${var.proxmox_node}"

  started = true

  agent {
    enabled = true
  }


  initialization {
    datastore_id = "local-zfs"
    user_account {
      # do not use this in production, configure your own ssh key instead!
      username = "fanat"
      keys     = ["ssh-rsa "]
    }
    ip_config {
      ipv4 {
        address = "192.168.1.9${count.index}/24"
        gateway = "192.168.1.1"
      }
    }
  }

  cpu {
    cores = 8
  }
  memory {
    dedicated = 4096
  }
  network_device {}

  disk {
    datastore_id = "local-zfs"
    file_format = "raw"
    file_id      = proxmox_virtual_environment_file.ubuntu_cloud_image.id
    interface    = "virtio0"
    iothread     = true
    discard      = "on"
    size         = 100
  }

}

resource "proxmox_virtual_environment_vm" "fanat-gitlab-runner" {
  count = 2
  
  name      = "fanat-gitlab-runner-${count.index}"
  node_name = "${var.proxmox_node}"
  
  started = true

  agent {
    enabled = true
  }


  initialization {
    datastore_id = "local-zfs"
    user_account {
      # do not use this in production, configure your own ssh key instead!
      username = "fanat"
      keys     = ["ssh-rsa "]
    }
    ip_config {
      ipv4 {
        address = "192.168.1.9${count.index + 1}/24"
        gateway = "192.168.1.1"
      }
    }
  }

  cpu {
    cores = 4
  }
  memory {
    dedicated = 2048
  }
  network_device {}

  disk {
    datastore_id = "local-zfs"
    file_format = "raw"
    file_id      = proxmox_virtual_environment_file.ubuntu_cloud_image.id
    interface    = "virtio0"
    iothread     = true
    discard      = "on"
    size         = 20
  }

}
